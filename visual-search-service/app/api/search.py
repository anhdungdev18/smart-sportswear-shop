...
async def search_by_image(
    ...
    try:
        model = await repository.active_model()
        if model is None:
        ...
        candidates = await repository.search(model.id, result.vector, limit)
        await repository.record_query_usage(model, result, latency_ms)
