-- V16: Ensure all tables from V3 exist on Supabase cloud.
-- V14 and V15 were already recorded in flyway_schema_history with different
-- SQL (from a prior team state), so their CREATE statements never ran.
-- This migration uses IF NOT EXISTS on every statement - completely safe to
-- run on databases that already have some or all of these tables.

-- promotions (parent of coupons)
...
-- coupons (parent of coupon_usages)
...
-- coupon_usages (THE missing table causing Hibernate validation failure)
