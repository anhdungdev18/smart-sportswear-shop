-- V14: Re-create tables from V3 that are missing on Supabase cloud.
-- Uses CREATE TABLE IF NOT EXISTS so this is safe to run even if some
-- tables already exist (e.g. on a local DB that ran V3 fully).
-- Root cause: V3 was partially applied on Supabase and coupon_usages
-- (and dependent objects) were never created.

-- Dependencies for coupon_usages: promotions, coupons must exist first.
-- Those were created in the first half of V3 that DID apply, so we only
-- need to create the tables that were missing.

-- ============================================================================
-- promotions (guard - may already exist)
-- ============================================================================
...
create index if not exists idx_coupons_promotion_id on coupons (promotion_id);
create index if not exists idx_coupons_status on coupons (status);

-- ============================================================================
-- coupon_usages (THE missing table that caused startup failure)
-- ============================================================================
...
create index if not exists idx_shipments_shipping_method_id on shipments (shipping_method_id);
...
create index if not exists idx_returns_handled_by on returns (handled_by);
...
create index if not exists idx_banner_items_product_id on banner_items (product_id);
