...
@Component
public class ProductSearchRateLimiter {
    ...
    public ProductSearchRateLimiter(ProductSearchProperties properties) {
    ...
    public void check(String clientKey) {
        ...
        if (updated.count() > Math.max(1, properties.rateLimitPerMinute())) {
            throw new BusinessRuleException(HttpStatus.TOO_MANY_REQUESTS, "Search rate limit exceeded");
        }
    }
}
