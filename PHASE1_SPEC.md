...
- Email giao dịch: Resend hoặc Nodemailer SMTP
...
- Refresh token phải có cơ chế revoke. Phase 1 lưu refresh token dạng hash trong bảng `refresh_tokens`; có thể dùng Redis làm cache hoặc blacklist cho việc revoke nhanh, nhưng `refresh_tokens` ở Postgres là nguồn dữ liệu chính
- Quên mật khẩu dùng email chứa liên kết hoặc mã đặt lại mật khẩu
- Reset password token phải được lưu dạng hash trong bảng `password_reset_tokens`
- `password_reset_tokens` phải có TTL rõ ràng, đề xuất 15 phút
- Mỗi token reset chỉ được dùng một lần
- Phải rate limit cho API quên mật khẩu theo email và IP để tránh spam
...
- Trong Phase 1, mỗi user chỉ có đúng 1 role tại một thời điểm
- Admin có thể đổi role của user, nhưng không có multi-role trong Phase 1
...
- Không xóa hard delete nếu đã phát sinh đơn, chỉ soft delete hoặc hide
...
- Hỗ trợ guest cart: khách chưa đăng nhập vẫn thêm được vào giỏ, giỏ được định danh bằng `session_id` qua cookie, không gắn `user_id`
- Khi guest đăng nhập hoặc đăng ký thành công, hệ thống phải merge guest cart vào cart của user: nếu cùng variant thì cộng dồn số lượng nhưng không vượt tồn khả dụng, nếu khác variant thì thêm mới; sau khi merge, xóa guest cart
- Bảng `carts` cần có cột `session_id` nullable bên cạnh `user_id` nullable để hỗ trợ cả 2 trường hợp

API contract bổ sung:

- `GET /api/v1/cart` khi chưa đăng nhập phải đọc giỏ theo `session_id` từ cookie
- `POST /api/v1/cart/items`, `PATCH /api/v1/cart/items/:id`, `DELETE /api/v1/cart/items/:id` khi chưa đăng nhập cũng thao tác theo `session_id`
- Sau `login/register` thành công, backend phải tự động merge guest cart vào user cart trước khi trả response hoàn tất đăng nhập
...
- Callback có thể bị cổng thanh toán gọi lại nhiều lần với cùng `transaction_ref`; xử lý idempotent, nếu đã xử lý rồi thì trả thành công ngay mà không update lại order hoặc payment
...
- `Gross revenue`: tính theo đơn có `payment_status = PAID`
- `Realized revenue`: tính theo đơn có `order_status = DELIVERED`
- Dashboard overview Phase 1 mặc định hiển thị cả 2 số để tránh hiểu sai doanh thu
...
3. Chọn variant
4. Nhập kho, xuất kho hoặc điều chỉnh
5. Ghi lý do
...
- Ảnh có thể gắn product hoặc variant, nhưng Phase 1 ưu tiên product image

### 9.2 Tồn kho

- Tồn khả dụng = `stock_quantity - reserved_quantity`
- Cơ chế chốt cho Phase 1 là reserve khi tạo đơn, trừ thật khi xác nhận, khớp với `reserved_quantity` ở bảng `product_variants` và các loại transaction ở mục 8.6:
...
2. Khi đơn được xác nhận (`CONFIRMED`): giảm `stock_quantity` và `reserved_quantity` tương ứng, ghi `inventory_transactions` với type `ORDER_CONFIRM_DEDUCT`
3. Khi đơn bị hủy ở trạng thái còn đang giữ chỗ: giảm `reserved_quantity`, ghi `inventory_transactions` với type `ORDER_RELEASE`
4. Tồn khả dụng dùng để validate khi thêm giỏ hàng và khi tạo đơn là `stock_quantity - reserved_quantity`
- Mọi thay đổi `stock_quantity` hoặc `reserved_quantity` đều phải nằm trong 1 DB transaction và phải có dòng tương ứng trong `inventory_transactions`
...
- `role` là enum với 4 giá trị cố định: `CUSTOMER`, `SALES_STAFF`, `WAREHOUSE_STAFF`, `ADMIN`
- Phase 1 không dùng `roles` và `user_roles`
- Mỗi user chỉ có một role tại một thời điểm
...
- `payments.transaction_ref` phải unique
- `product_variants.sku` phải unique
- `users.email` phải unique
- `carts.user_id` unique khi không null để mỗi user chỉ có 1 cart active
- `carts.session_id` unique khi không null để mỗi guest session chỉ có 1 cart active
...
- `forgot-password` luôn trả response thành công chung, không tiết lộ email có tồn tại hay không
- `reset-password` yêu cầu token hợp lệ, chưa hết hạn và chưa dùng
...
- Price > 0
- Quantity > 0 với import hoặc export
...
- Không cho `reserved_quantity` âm
- Không cho `stock_quantity` âm
...
- 1 sales staff
- 1 warehouse staff
- 10 categories hoặc brands cơ bản
- 20-30 products
- 50+ variants
...
- Guest cart và merge cart sau login
- Thêm vào giỏ
- Tạo đơn
- Reserve tồn khi tạo đơn
- Confirm đơn và trừ tồn thật
- Cancel đơn và release tồn
...
- Luồng guest cart sang user cart
- Luồng quên mật khẩu
...
- Guest cart hoạt động đúng và merge đúng sau login
- Quên mật khẩu hoạt động qua email thật hoặc SMTP staging
...
- Báo cáo overview hiển thị đúng gross revenue và realized revenue
...
5. Cart và guest cart
6. Orders
7. Payments
8. Inventory
9. Forgot password
10. Admin dashboard và reports
11. Hardening, seed, test, docs
