...
import { activateVisualSearchModel, backfillMissingEmbeddings, getVisualSearchDashboard, reindexVisualSearch, retryFailedEmbeddings } from "@/modules/visual-search/browser-api";
import type { VisualSearchCoverage, VisualSearchJobs, VisualSearchModels, VisualSearchOperations, VisualSearchUsage } from "@/modules/visual-search/types";

type Dashboard = { coverage: VisualSearchCoverage; operations: VisualSearchOperations; usage: VisualSearchUsage; jobs: VisualSearchJobs; models: VisualSearchModels };
...
export function VisualSearchOperationsClient() {
  ...
  async function activateModel(id: string) {
    if (!window.confirm("Chuyển model này thành ACTIVE? Coverage phải đạt ít nhất 98%.")) return;
    setRetrying(true); setNotice("");
    try { await activateVisualSearchModel(id); setNotice("Đã chuyển model ACTIVE thành công."); await load(); }
    catch { setNotice("Không thể chuyển model; hãy kiểm tra coverage tối thiểu 98%."); }
    finally { setRetrying(false); }
  }
  ...
  return <>
    ...
    <section className="card panel"><div className="panel-header"><h2>Ngân sách và model versions</h2></div><p className={operations.budgetUsagePct >= 70 ? "trend warn" : "trend"}>${operations.monthlyCostUsd.toFixed(4)} / ${operations.monthlyBudgetUsd.toFixed(2)} tháng này ({operations.budgetUsagePct.toFixed(1)}%){operations.budgetExhausted ? " — ĐÃ HẾT NGÂN SÁCH" : ""}</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Model</th><th>Trạng thái</th><th>Coverage</th><th>Kích hoạt / rollback</th></tr></thead><tbody>{data.models.models.map((item) => <tr key={item.id}><td>{item.provider} / {item.model} r{item.revision} ({item.dimensions})</td><td>{item.status}</td><td>{item.readyImageCount}/{item.targetImageCount}</td><td><button className="admin-btn secondary" disabled={retrying || item.status === "ACTIVE"} onClick={() => void activateModel(item.id)}>Kích hoạt</button></td></tr>)}</tbody></table></div></section>
