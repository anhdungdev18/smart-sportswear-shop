    ...
    async def execute(self, name: str, token: str, args: dict[str, Any]):
        ...
        result = {
            ...
            "get_inventory_risk_explanation": {
                ...
                "lookup": [{"sku": "SKU-1", "availableQuantity": 4, "stockQuantity": 5, "reservedQuantity": 1}],
...
def test_react_graph_executes_multi_tool_readonly_workflow():
    ...
    assert [name for name, _ in registry.calls] == ["get_inventory_risk_explanation", "get_forecast_quality"]
    assert state["selected_tool"] == "get_inventory_risk_explanation"
    ...
    assert "suggestedQuantity=30" in state["grounded_numbers"]
